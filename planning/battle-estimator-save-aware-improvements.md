...
- Done in commit for T10. Added a right-sidebar `Alerts` section above
  `Target`, backed by the `/api/state` alert contract. The renderer shows
  distinct diagnostic states for unconfigured color, unavailable ownership, no
  owned towns, no threats, unknown status, and snapshot errors. OK payloads
  render a compact non-clickable alert list with enemy hero, enemy color
  swatch/name, distance, nearest owned town, and `+N other towns in radius`
  detail. Rows carry alert and enemy hero IDs for T11, but have no button role,
  tabindex, or click handlers yet. Added overflow-safe list styling and tests
  for section ordering, diagnostics, row content, inert behavior, color swatch
  rendering, and ARIA role cleanup when returning from list state to diagnostic
  state.
...
- Added `HeroCombatContext` and `HeroPrimarySkills` on parsed `HeroArmy`
  records, with `primary-only` status for the bounded supported case.
- Primary combat context is supported only for loaded `.GM1` saves with
  `H3SVG` at offset `0` and raw `0x00` hero records. Unsupported structures,
  shifted `H3SVG`, XOR `0x01`, `GM2`, and default byte-only scans return
  explicit `unavailable` reasons instead of fallback values.
- Documented the T16 support gate in
  `tools/battle_estimator_save_parsing_checkpoint.md`.
- Verified with `python3 -m unittest tests.test_h3_save_parser`,
  `python3 -m unittest tests.test_battle_estimator_gui`, and
  `git diff --check`.
...
- Added validated secondary-skill decoding for the same bounded
  `.GM1`/`H3SVG=0`/raw `0x00` hero combat structure as T16.
- Complete secondary vectors now produce `primary+secondary` context with
  save-derived `CurrentSkill` values sorted by hero-screen slot. Zero secondary
  skills are represented as an empty tuple, not as VCMI starting-skill fallback.
- Invalid secondary count, slot, level, level/slot mismatch, metadata mapping,
  and truncation cases degrade to `primary-only` with explicit reasons while
  preserving parsed primary skills.
- Documented the T17 parser behavior in
  `tools/battle_estimator_save_parsing_checkpoint.md`.
- Verified with
  `python3 -m unittest tests.test_h3_save_parser tests.test_hero_skill_recommender`
  and `git diff --check`.
...
- Added an explicit `source` field to `HeroCombatContext`; only the
  loaded-save parser path can mark combat context as `save` derived.
- Added the `partial` status to the internal contract, source-aware passive
  modifier summaries, and stable modifier keys for Offence, Armorer, and
  Archery.
- GUI hero snapshots now serialize `combat_context` with status, source,
  reason, primary skills, secondary skills, passive modifiers, and an empty
  `unsupported_observed` placeholder.
- Switched GUI snapshot hero scanning to the loaded-save-aware parser so
  supported `.GM1` saves expose save-derived combat context while unsupported
  layouts remain `unavailable`.
- Verified with `python3 -m unittest tests.test_h3_save_parser
  tests.test_battle_estimator_gui` and `git diff --check`.
...
- Added `CombatSideModifiers` and wired save-derived primary
  Attack/Defense plus Offence, Armorer, and Archery into Monte Carlo damage
  calculation.
- Limited combat context application to loaded-save-derived contexts; missing
  or unsupported contexts still run with army-only estimates.
- Applied selected hero context to neutral and hero target estimates, enemy
  hero context to hero-vs-hero estimates, and the same context plumbing to CLI
  autosave analysis branches.
- Switched CLI nearby scan to the loaded-save-aware hero parser so supported
  `.GM1` saves carry combat context into estimates.
- Added focused damage-order, context-plumbing, CLI, and GUI regression tests.
...
## Hero Ranking Combat Score Follow-Up

These tasks replace the misleading raw-AI primary sort in the GUI Hero Ranking
dialog with a save-local combat score. A hero's combat score is the average
estimated win percentage when attacking every other positioned hero with an
army in the currently loaded save. Same-owner and same-team heroes are included
because the ranking is a global strength comparison, not a hostile-target scan.
...
| T16 | Parse Hero Primary Skills | done | T15 | hero-combat-parser | Main | M | Core | `tools/h3_save_parser.py`, `tests/test_h3_save_parser.py` |
